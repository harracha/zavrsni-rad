...
export const listStudentHomeworks = async (student: string | Student): Promise<Homework[]> => {
    try{
...
export const getBatchHomeworks = async (studentIds: string[]): Promise<Homework[]> => {
    try{
        const homeworks = await prisma.homework.findMany({
            where: {
...
export const listHomeworks = async(queryParams: homeworkFilterParams): Promise<Homework[]> => {
...
export const updateHomework = async(data: Prisma.HomeworkUncheckedUpdateManyInput) => {
...
export const filterExistingHomeworks = async (data: Prisma.HomeworkUncheckedCreateInput[]) => {
    ...
    try {
        ...
        homeworksCopy.map((homework: Homework) => {
            ...
            const existingHomework = studentHomeworks.find((studentHomework) => studentHomework.acYear === homework.acYear && studentHomework.name === homework.name && studentHomework.studentId === homework.studentId)
        ...
        return await Promise.all([createHomework(homeworks as unknown as  Prisma.HomeworkCreateManyInput), updateHomework(existingHomeworks as unknown as Prisma.HomeworkUncheckedUpdateManyInput)])
